...
export default function EditarPQRSPage() {
  const [form, setForm] = useState({
    tipo: pqrs.tipo,
    asunto: pqrs.asunto,
    descripcion: pqrs.descripcion,
    adjunto: pqrs.adjunto,
  });
  const [fileName, setFileName] = useState(pqrs.adjunto);
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setForm((f) => ({ ...f, adjunto: file ? file.name : "" }));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setError("");
    alert("Edición guardada (simulado)");
    router.push(`/pqrs/${pqrs.id}`);
  };
  return (
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 flex justify-center items-start py-12 px-2 sm:px-8 bg-white rounded-tl-3xl rounded-bl-3xl min-h-[80vh]">
          <form onSubmit={handleSubmit} className="w-full max-w-xl flex flex-col gap-6">
            <h1 className="text-2xl font-semibold mb-2">Editar PQRS</h1>
            ...
            <div className="flex justify-end">
              <button type="submit" className="bg-[#b9a4f4] text-white px-6 py-2 rounded hover:bg-[#a18be6] transition">Confirmar edición</button>
            </div>
          </form>
        </main>
      </div>
